...
def main():
    ...
    # Validacuion de min_genes
    if min_genes < 0:
        print("Error: --min_genes debe ser un número entero no negativo")
        exit(1)
    ...
    try:
        ...
        print(f"Error: no se tiene permiso para leer el archivo -> {input_file}")
        exit(1)
    ...
    if not interactions:
        print("Advertencia: no se encontraron interacciones válidas en el archivo.")
        exit(0)
    ...
    if not regulon:
        print(f"Advertencia: no se encontraron TFs con al menos {min_genes} genes regulados.")
        exit(0)
    ...
    try:
        ...
        print(f"Error: el archivo no existe -> {output_file}")
        exit(1)
    
    except PermissionError:
        print(f"Error: no se tiene permiso para escribir el archivo -> {output_file}")
        exit(1)
